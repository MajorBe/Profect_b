...
@export var enable_approach_chaos: bool = false
...
# ========================= #   Chaos: Tick # =========================
func _tick_chaos_once(dt: float, _player_pos: Vector3, default_stop_pos: Vector3) -> void:
	...
	# Принимаем решение только из idle
	if _chaos_state == "idle" and _chaos_decision_cd_left <= 0.0 and _chaos_idle_grace_left <= 0.0:
		...
		if r < p_pause_dt:
			...
			# ===== BACKSTEP → обычный EVADE =====
			_change_state(Cossack_01_EvadeState.new(self))
			# применяем смену состояния сразу в этот же кадр (без ожидания deferred)
			if _pending_state != null and not _switching_now:
				_apply_pending_state()

			# ставим grace и cooldown для хаоса, чтобы не триггерить подряд
			_chaos_state = "idle"
			_chaos_idle_grace_left = idle_grace_val
			_chaos_decision_cd_left = _chaos_decision_cooldown
			_chaos_backstep_steps_left = 0
			_chaos_timer = 0.0

			# цель на этот кадр — текущая позиция тела; дальше движение возьмёт EvadeState
	...
	match _chaos_state:
		"paused":
			...
			if _chaos_timer <= 0.0:
				...
				_chaos_idle_grace_left = idle_grace_val
				_chaos_decision_cd_left = _chaos_decision_cooldown
			# в паузе стоим на месте
			_chaos_cached_target = (Body.global_position if Body != null else default_stop_pos)

		"backstep":
			# Сюда больше не зайдём (backstep перенаправлен в Evade), оставлено для совместимости
			_chaos_timer -= dt
			if _chaos_timer <= 0.0:
