...
@export_group("Debug")
@export var debug_jump_ai: bool = false
...
@export var debug_gap_calc: bool = false
...
var _probe_interval: float = 0.1  # Настраиваемый интервал для raycasts
var _probe_cache: Dictionary = {}  # Кэш: {"key": {"value": val, "time": physics_frames}}
...
func _ready() -> void:
	...
	var floor_ahead: RayCast3D = body.get_node_or_null("floor_ahead")
	var wall_ahead: RayCast3D = body.get_node_or_null("wall_ahead")
	if floor_ahead:
		floor_ahead.enabled = true
		floor_ahead.collision_mask = ground_layer_mask | platform_layer_mask
	if wall_ahead:
		wall_ahead.enabled = true
		wall_ahead.collision_mask = ground_layer_mask | platform_layer_mask
...
func setup(_actor: Node3D, _body: CharacterBody3D, _cfg: Variant) -> void:
	...
	if actor != null:
		for item_name in ["Config", "config", "enemy_config"]:
...
func _physics_process(delta: float) -> void:
	...
	# OPT: Throttling для всех тяжелых проб
	_probe_timer += delta
	if _probe_timer >= _probe_interval:
		_probe_timer = 0.0
		_try_wall_hop_towards_player()  # OPT: Только здесь
		_edge_safety_guard()  # OPT: Только здесь
# =============================================================================
# ПУБЛИЧНОЕ API
# =============================================================================
func stop() -> void:
...
func _is_imminent_gap_ahead(forward: float, down: float) -> bool:
	...
	# OPT: Кэш
	var cache_key: String = "imminent_gap_ahead_" + str(forward) + "_" + str(down)
	if _probe_cache.has(cache_key) and Engine.get_physics_frames() - _probe_cache[cache_key]["time"] < 2:
		return _probe_cache[cache_key]["value"]

	# OPT: Используем floor_ahead для проверки впереди, но нужен raycast под собой (floor_here)
	var floor_ahead_ray: RayCast3D = body.get_node_or_null("floor_ahead")
	var floor_here_ray: RayCast3D = body.get_node_or_null("floor_here")  # Рекомендую добавить в сцену

	var hit_here: bool = false
	var hit_ahead: bool = false

	if floor_ahead_ray and floor_here_ray:
		# OPT: Настраиваем позиции и направления
		floor_here_ray.position = Vector3(0.0, 0.05, 0.0)
		floor_here_ray.target_position = Vector3(0.0, -down, 0.0)
		floor_ahead_ray.position = Vector3(_dirf() * forward, 0.05, 0.0)
		floor_ahead_ray.target_position = Vector3(0.0, -down, 0.0)
		floor_here_ray.force_raycast_update()
		floor_ahead_ray.force_raycast_update()
		hit_here = floor_here_ray.is_colliding()
		hit_ahead = floor_ahead_ray.is_colliding()
	else:
		# OPT: Фолбэк на ручной raycast, если нет floor_here
		...
		q_here.collide_with_bodies = true
		q_here.collide_with_areas = true
		q_here.exclude = [body.get_rid()]
		...
		q_ahead.collide_with_bodies = true
		q_ahead.collide_with_areas = true
		q_ahead.exclude = [body.get_rid()]

		var hit_here_dict: Dictionary = space.intersect_ray(q_here)
		var hit_ahead_dict: Dictionary = space.intersect_ray(q_ahead)

		hit_here = (hit_here_dict.size() > 0)
		hit_ahead = (hit_ahead_dict.size() > 0)

	var result: bool = hit_here and not hit_ahead
	_probe_cache[cache_key] = {"value": result, "time": Engine.get_physics_frames()}
	# OPT: Рекомендация: добавьте floor_here: RayCast3D в сцену для оптимизации
	return result

func _is_imminent_gap_in_dir(dir_sign: float, forward: float, down: float) -> bool:
	if body == null:
		return false

	# OPT: Кэш
	var cache_key: String = "imminent_gap_in_dir_" + str(dir_sign) + "_" + str(forward) + "_" + str(down)
	if _probe_cache.has(cache_key) and Engine.get_physics_frames() - _probe_cache[cache_key]["time"] < 2:
		return _probe_cache[cache_key]["value"]

	# OPT: floor_ahead для проверки впереди, нужен floor_here под себя
	var floor_ahead_ray: RayCast3D = body.get_node_or_null("floor_ahead")
	var floor_here_ray: RayCast3D = body.get_node_or_null("floor_here")
	...
	if floor_ahead_ray and floor_here_ray:
		floor_here_ray.position = Vector3(0.0, 0.05, 0.0)
		floor_here_ray.target_position = Vector3(0.0, -down, 0.0)
		floor_ahead_ray.position = Vector3(dir_sign * forward, 0.05, 0.0)
		floor_ahead_ray.target_position = Vector3(0.0, -down, 0.0)
		floor_here_ray.force_raycast_update()
		floor_ahead_ray.force_raycast_update()
		hit_here = floor_here_ray.is_colliding()
		...
		var from_ahead: Vector3 = origin + Vector3(dir_sign * forward, 0.05 * up_sign, 0.0)
		var to_ahead:   Vector3 = from_ahead + Vector3(0.0, -down * up_sign, 0.0)
	...
	var result: bool = hit_here and not hit_ahead
	_probe_cache[cache_key] = {"value": result, "time": Engine.get_physics_frames()}
	# OPT: Рекомендация: добавьте floor_here: RayCast3D для оптимизации
	return result
...
func _measure_gap_ahead(dirf: float, start_x: float, max_scan_x: float, down: float) -> Dictionary:
	...
	# OPT: Кэш
	var cache_key: String = "measure_gap_ahead_" + str(dirf) + "_" + str(start_x) + "_" + str(max_scan_x) + "_" + str(down)
	if _probe_cache.has(cache_key) and Engine.get_physics_frames() - _probe_cache[cache_key]["time"] < 5:
		return _probe_cache[cache_key]["value"]

	var space: PhysicsDirectSpaceState3D = body.get_world_3d().direct_space_state
	var origin: Vector3 = body.global_transform.origin
	var up_sign: float = (1.0 if uses_positive_up() else -1.0)
	var step: float = max(0.1, gap_scan_step)  # OPT: Увеличенный шаг
	...
	var _step_drop_thresh: float = 0.20

	# OPT: Используем floor_ahead для сканирования
	var floor_ahead_ray: RayCast3D = body.get_node_or_null("floor_ahead")
	...
	while x <= max_scan_x + 0.0001:
		...
		var floor_y: float = self_y

		if floor_ahead_ray:
			floor_ahead_ray.position = Vector3(dirf * x, 0.05, 0.0)
			floor_ahead_ray.target_position = Vector3(0.0, -down, 0.0)
			floor_ahead_ray.force_raycast_update()
			has_floor = floor_ahead_ray.is_colliding()
			if has_floor:
				floor_y = floor_ahead_ray.get_collision_point().y
		else:
			...
			has_floor = (hit.size() > 0)
			if has_floor:
				floor_y = (hit.get("position", origin) as Vector3).y

		var is_gap_x: bool = (not has_floor) or ((self_y - floor_y) >= _step_drop_thresh)
		...
		if is_gap_x:
			...
			if has_floor:
				if uses_positive_up():
					min_y_in_gap = min(min_y_in_gap, floor_y)
				else:
					min_y_in_gap = max(min_y_in_gap, floor_y)
		else:
			if gap_begin_x >= 0.0:
				gap_end_x = x
				break
	...
	if gap_begin_x >= 0.0 and gap_end_x < 0.0:
		gap_end_x = max_scan_x

	if gap_begin_x >= 0.0 and gap_end_x > gap_begin_x:
		var land_x: float = gap_end_x
		var from_p2: Vector3 = origin + Vector3(dirf * land_x, 0.05 * up_sign, 0.0)
		var to_p2:   Vector3 = from_p2 + Vector3(0.0, -down * up_sign, 0.0)

		if floor_ahead_ray:
			floor_ahead_ray.position = Vector3(dirf * land_x, 0.05, 0.0)
			floor_ahead_ray.target_position = Vector3(0.0, -down, 0.0)
			floor_ahead_ray.force_raycast_update()
			if floor_ahead_ray.is_colliding():
				land_found = true
				land_y = floor_ahead_ray.get_collision_point().y
		else:
			...
			q2.exclude = [body.get_rid()]
			var hit2: Dictionary = space.intersect_ray(q2)
			if hit2.size() > 0:
				land_found = true
				land_y = (hit2.get("position", origin) as Vector3).y

	result["has_gap"] = (gap_begin_x >= 0.0 and gap_end_x > gap_begin_x)
	result["width"] = max(0.0, gap_end_x - gap_begin_x)
	result["land_y"] = land_y
	result["land_found"] = land_found
	result["min_y"] = min_y_in_gap

	_probe_cache[cache_key] = {"value": result.duplicate(true), "time": Engine.get_physics_frames()}
	return result
...
func _probe_lip_height_ahead(forward_offset: float = 0.6, probe_up: float = 0.9, probe_down: float = 2.5) -> Variant:
	...
	# OPT: Кэш
	var cache_key: String = "probe_lip_height_" + str(forward_offset) + "_" + str(probe_up) + "_" + str(probe_down)
	if _probe_cache.has(cache_key) and Engine.get_physics_frames() - _probe_cache[cache_key]["time"] < 3:
		return _probe_cache[cache_key]["value"]

	# OPT: Нужен отдельный RayCast3D (lip_ray), так как floor_ahead не подходит (разный collision_mask)
	var lip_ray: RayCast3D = body.get_node_or_null("lip_ray")  # Рекомендую добавить

	if lip_ray:
		lip_ray.position = Vector3(_dirf() * forward_offset, probe_up, 0.0)
		lip_ray.target_position = Vector3(0.0, -probe_down, 0.0)
		lip_ray.collision_mask = ground_layer_mask
		lip_ray.force_raycast_update()
		if lip_ray.is_colliding():
			var pos: Vector3 = lip_ray.get_collision_point()
			_dbg_probe("LIP probe hitY=%.3f" % pos.y)
			_probe_cache[cache_key] = {"value": pos.y, "time": Engine.get_physics_frames()}
			return pos.y
		else:
			_dbg_probe("LIP probe hit=none")
			_probe_cache[cache_key] = {"value": null, "time": Engine.get_physics_frames()}
			return null
	else:
		var space: PhysicsDirectSpaceState3D = body.get_world_3d().direct_space_state
...
func _probe_platform_top_ahead(forward_offset: float = 0.6, probe_up: float = 1.2, probe_down: float = 3.5) -> Variant:
	...
	# OPT: Кэш
	var cache_key: String = "probe_platform_top_" + str(forward_offset) + "_" + str(probe_up) + "_" + str(probe_down)
	if _probe_cache.has(cache_key) and Engine.get_physics_frames() - _probe_cache[cache_key]["time"] < 3:
		return _probe_cache[cache_key]["value"]

	# OPT: Используем floor_ahead, так как platform_layer_mask совпадает
	var floor_ahead_ray: RayCast3D = body.get_node_or_null("floor_ahead")

	if floor_ahead_ray:
		floor_ahead_ray.position = Vector3(_dirf() * forward_offset, probe_up, 0.0)
		floor_ahead_ray.target_position = Vector3(0.0, -probe_down, 0.0)
		floor_ahead_ray.collision_mask = platform_layer_mask
		floor_ahead_ray.force_raycast_update()
		if floor_ahead_ray.is_colliding():
			var pos: Vector3 = floor_ahead_ray.get_collision_point()
			_dbg_probe("PLAT probe hitY=%.3f" % pos.y)
			_probe_cache[cache_key] = {"value": pos.y, "time": Engine.get_physics_frames()}
			return pos.y
		else:
			_dbg_probe("PLAT probe hit=none")
			_probe_cache[cache_key] = {"value": null, "time": Engine.get_physics_frames()}
			return null
	else:
		var space: PhysicsDirectSpaceState3D = body.get_world_3d().direct_space_state
...
func _is_wall_ahead(forward_offset: float = 0.5, chest_height: float = 0.9) -> bool:
	...
	# OPT: Кэш
	var cache_key: String = "is_wall_ahead_" + str(forward_offset) + "_" + str(chest_height)
	if _probe_cache.has(cache_key) and Engine.get_physics_frames() - _probe_cache[cache_key]["time"] < 2:
		return _probe_cache[cache_key]["value"]

	# OPT: Используем wall_ahead
	var wall_ahead_ray: RayCast3D = body.get_node_or_null("wall_ahead")

	if wall_ahead_ray:
		wall_ahead_ray.position = Vector3(0.0, chest_height, 0.0)
		wall_ahead_ray.target_position = Vector3(_dirf() * forward_offset, 0.0, 0.0)
		wall_ahead_ray.collision_mask = ground_layer_mask | platform_layer_mask
		wall_ahead_ray.force_raycast_update()
		var hit: bool = wall_ahead_ray.is_colliding()
		_dbg_probe("WALL probe hit=%s" % str(hit))
		_probe_cache[cache_key] = {"value": hit, "time": Engine.get_physics_frames()}
		return hit
	else:
		var space: PhysicsDirectSpaceState3D = body.get_world_3d().direct_space_state
		...
		q.exclude = [body.get_rid()]
		var res: Dictionary = space.intersect_ray(q)
		var hit: bool = not res.is_empty()
		_dbg_probe("WALL probe from=%s to=%s hit=%s" % [str(origin), str(to_point), str(hit)])
		_probe_cache[cache_key] = {"value": hit, "time": Engine.get_physics_frames()}
		return hit
...
func _has_headroom_at(target_xy: Vector3, up_clearance: float = 1.2) -> bool:
	...
	# OPT: Кэш
	var cache_key: String = "has_headroom_at_" + str(target_xy) + "_" + str(up_clearance)
	if _probe_cache.has(cache_key) and Engine.get_physics_frames() - _probe_cache[cache_key]["time"] < 3:
		return _probe_cache[cache_key]["value"]

	# OPT: Нужен headroom_ray, так как это динамическая позиция
	var headroom_ray: RayCast3D = body.get_node_or_null("headroom_ray")  # Рекомендую добавить

	if headroom_ray:
		headroom_ray.global_position = target_xy
		headroom_ray.target_position = Vector3(0.0, up_clearance, 0.0)
		headroom_ray.collision_mask = ground_layer_mask | platform_layer_mask
		headroom_ray.force_raycast_update()
		var clear: bool = not headroom_ray.is_colliding()
		_dbg_probe("HEADROOM probe clear=%s" % str(clear))
		_probe_cache[cache_key] = {"value": clear, "time": Engine.get_physics_frames()}
		return clear
	else:
		var space: PhysicsDirectSpaceState3D = body.get_world_3d().direct_space_state
...
# =============================================================================
func _has_need_overcome(_dir_sign: float = 0.0) -> bool:
	...
	var self_y: float = body.global_position.y
	var lip_v: Variant = _probe_lip_height_ahead(0.6, 0.9, 2.5)  # явный тип, без :=
...
func _can_reach_y_with_jumps(self_y: float, target_y: float, _jumps_available: int = 1, extra_margin: float = 0.20) -> bool:
	var gap: float = absf(target_y - self_y)
	return gap <= (max_jump_height_estimate() + extra_margin)
...
func _should_turn_around() -> bool:
	...
	if acc >= _probe_interval:
		acc = 0.0
		var origin: Vector3 = body.global_transform.origin
		var dirx: float = float(actor.facing_dir)

		# OPT: Используем floor_ahead и wall_ahead
		var floor_ahead_ray: RayCast3D = body.get_node_or_null("floor_ahead")
		var wall_ahead_ray: RayCast3D = body.get_node_or_null("wall_ahead")

		var floor_ok: bool = false
		var wall_block: bool = false

		if floor_ahead_ray and wall_ahead_ray:
			floor_ahead_ray.position = Vector3(dirx * patrol_turn_forward, 0.2, 0.0)
			floor_ahead_ray.target_position = Vector3(0.0, -(patrol_turn_down - 0.2), 0.0)
			wall_ahead_ray.position = Vector3(dirx * 0.6, 0.5, 0.0)
			wall_ahead_ray.target_position = Vector3(dirx * 0.5, 0.0, 0.0)
			floor_ahead_ray.force_raycast_update()
			wall_ahead_ray.force_raycast_update()
			floor_ok = floor_ahead_ray.is_colliding()
			wall_block = wall_ahead_ray.is_colliding()
		else:
			...
			floor_ok = not res_floor.is_empty()
			...
			wall_block = not res_wall.is_empty()
